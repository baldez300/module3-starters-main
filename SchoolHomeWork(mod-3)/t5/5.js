...
// add your code here
//document.body.innerHTML = picArray.reduce((prev, elem) => prev + `<h2>${elem.title}</h2> <img src="
//${elem.image.medium}" alt="${elem.title}" /> <figcaption>${elem.caption}</figcaption> <p>${elem.description}</p>`);
'use strict';
...
for (let pic of picArray){
  ...
  h2.append(pic.title)
  im.src = pic.image.medium;
  im.alt = pic.title;
  figcaption.append(pic.caption);
  //figcaption.textContent = pic.caption; // Toinen tapa
  p.append(pic.description)

  figure.append(im, figcaption);
